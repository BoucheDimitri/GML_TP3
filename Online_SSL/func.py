...
face_haar_cascade = cv.CascadeClassifier(path + "/data/haarcascade_frontalface_default.xml")
eye_haar_cascade = cv.CascadeClassifier(path + "/data/haarcascade_eye.xml")



def create_user_profile(user_name, faces_path = os.getcwd() + "/data/"):
...
def load_profile(user_name, faces_path = path + "/data/"):
    assert ("faces" in os.listdir(faces_path)), "Error : 'faces' folder not found"
    ## Check if profile exists. If not, create it.
    faces_path = os.path.join(faces_path, "faces")
    profile_path = os.path.join(faces_path, user_name)
...
class incremental_k_centers:  
    ...
    # No output, update self   
        ...
        if self.centroids.shape[0] >= self.max_num_centroids + 1:
            ...
            minimal_edge = np.argwhere(self.centroids_distances <= min_dist)[0]
            #################################################################  
            #################################################################     
            ...
            #################################################################     
            if minimal_edge[0] in self.taboo:
                c_rep, c_add = minimal_edge[0], minimal_edge[1]
            elif minimal_edge[1] in self.taboo:
                c_rep, c_add = minimal_edge[1], minimal_edge[0]
            elif self.V[minimal_edge[0]] <= self.V[minimal_edge[1]]:
                c_rep, c_add = minimal_edge[0], minimal_edge[1]
            else:
                c_rep, c_add = minimal_edge[1], minimal_edge[0]
...
#######################################################################    
# resize the face
    center = (int(grey_face.shape[0] / 2), int(grey_face.shape[1] / 2))
    grey_face = grey_face[center[0] - side: center[0] + side, center[1] - side: center[1] + side]
    grey_face = grey_face.reshape(EXTR_FRAME_SIZE * EXTR_FRAME_SIZE)
# scale the data to [0,1]
    grey_face = grey_face / 255
    return grey_face        
